...
# nlcd.df3 <- as.data.frame(FLnlcd2, xy=FALSE)
# nlcd.df4 <- as.data.frame(FLnlcd2, xy=TRUE)
# nlcd.xy2 <- data.frame(x=nlcd.df4$x,y=nlcd.df4$y)
# ##Cast over to SPxDF
# habitat2 <- SpatialPixelsDataFrame(nlcd.xy2,nlcd.df3)
# 
# # #Create an ltraj trajectory object. 
#  ltraj <- as.ltraj(st_coordinates(cat.albers), cat.albers$DT, id = cat.albers$Sex, burst = cat.albers$Sex, 
#    typeII = TRUE)
...
crs(habitat) <- st_crs(FLnlcd)$proj4string#"NAD83 / Conus Albers"
##Cast over to SPxDF
habitat2 <- SpatialPixelsDataFrame(rc.xy,rc.df2)
...
#Diffusion component for each habitat type using plug-in method
vv2<- BRB.D(ltraj, Tmax = tmax, Lmin = lmin,  habitat = habitat)
vv2
...
##Cast over to SPxDF
habitat3 <- SpatialPixelsDataFrame(rc1.xy,rc1.df2)
