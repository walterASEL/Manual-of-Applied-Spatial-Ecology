...
# #First create random sample of points in each polygon
# 
# ranbuff <- st_sample(settbuff.df,size=c(3,3),exact = T, type="random", by_polygon = TRUE)
...
#buffpt_id <- as.data.frame(ranbuff)

#p1_sf = st_sf(ranbuff)
#p1_joined = st_intersects(ranbuff, settbuff.df)
...
#xrand <- rep(c(1,2,3),length(settbuff))
#xrand$id <- 

#ranbuff.merged <- do.call('rbind', ranbuff)

# generate a reconstituted vector of animal IDs
#deer_id <- nrow(ranbuff.merged, deer.albers$id)

#buffpt_id <- paste(substr(deer_id, ranbuffwIDs), sep="_")
...
#buff_ids <- paste(settbuff.df$id, buff_ids, sep="_")
#Determine the number of ACTUAL sample points generated for each polygon
...
#Grab coordinates of all points
